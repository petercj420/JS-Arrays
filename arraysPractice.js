...
//loops through the given array backwards alerting every item in the array starting at the 
//end.
...
//(all the even values from nums) and the second item in the Array being the odds array(all 
//  the odd values from nums).
...
//and an array full of numbers. Your job is to write a function named finder that will get a 
//random number, then loop through the array to see if that random number is in the array. 
//If it is, return true, if it's not, return false

  //Code Here
function finder(){
  var randa = getRandomArbitrary();
  var boo = false;
...
//Write a function called reverse that takes a given str as it's only argument and returns 
...
/*
  ...
  //off of our grocery list,
  and adding new items to our list. 
  
  Write a function called removeItem that is given two arguments, the first is myGroceryList, 
  //and the
  second is an item to remove from myGroceryList. If the second argument (or the item to add 
  //or remove) matches an item in myGroceryList,
  ...
  and the second is an item to add to your grocery list. In addItem add the item you passed 
  //in to
...
//Write a function called maker that creates an array, fills that array with numbers from 1 
//to 215, then returns the array.
...
//new array after adding ten to each item in numbers. *Verify your answer is correct. 
//--> [15, 19, 26, 29, 35, 44, 58]
  ...
  function addTen(numbers){
    ...
    for (var x = 0; x < numbers.length; x++){
      var type = typeof(numbers[x]);
      if (type === "number")
      {
        numbers[x] += 10;
      }
      else if(type === "string"){
        var newnum = parseInt(numbers[x]);
        numbers[x] = newnum + 10;
      }
    }
    return numbers;
...
sure that it's equal to 4. */
  ...
  //Code Here
function addEmployees(devMountainEmployees) {
...
Loop through your devMountainEmployees until you find cahlan, then remove him from the array.*/
  ...
  //Code Here
function devMountainEmployeesfn(devMountainEmployees) {
  for (var x = 0; x < devMountainEmployees.length; x++){
    if(devMountainEmployees[x].name() === "Cahlan"){
      delete devMountainEmployees[x];
    }
  }
